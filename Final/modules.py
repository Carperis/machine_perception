...
def joint_attn_call2_0(
    ...
    **kwargs,
    ...
    # latent_size = patch_size * patch_size = 8 * 8 (when pipline height=width=128 px)
    # token_size = 77 + 77 = 154
    # print(hidden_states.shape)  # [batch, latent_size, attn_head*head_dim] -> [2, 64, 24*64]
    # print(encoder_hidden_states.shape)  # [batch, token_size, attn_head*head_dim] -> [2, 154, 24*64]
    ...
    key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
    value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
    # transpose to [batch, attn_head, latent_size, head_dim]
    ...
    if encoder_hidden_states is not None:
        ...
        # [batch, attn_head, latent_size, head_dim], [batch, attn_head, token_size, head_dim]
        # x_hidden_state_value: [2, 24, 64, 64], c_hidden_state_value: [2, 24, 154, 64]
        # print(value.shape, encoder_hidden_states_value_proj.shape)
        query = torch.cat([query, encoder_hidden_states_query_proj], dim=2)
        key = torch.cat([key, encoder_hidden_states_key_proj], dim=2)
        value = torch.cat([value, encoder_hidden_states_value_proj], dim=2) # [2, 24, 64+154, 64]
    ...
    if hasattr(self, "store_attn_map"):
        ...
        self.attn_map = rearrange(
            attention_probs,
            "batch attn_head (height width) token_size -> batch attn_head height width token_size", # latent_size = height*width
            height=height,
        )
        # print("attn_map", self.attn_map.shape)  # [2, 24, 8, 8, 154]
    ...
    # print("hidden_states (before reshape)", hidden_states.shape) # [batch_size, attn_head, latent_size+token_size, head_dim] -> [2, 24, 218, 64]
    hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
    # print("hidden_states (after reshape)", hidden_states.shape)  # [batch, latent_size+token_size, attn_head*head_dim] -> [2, 218, 1536]
    hidden_states = hidden_states.to(query.dtype)
    # print("hidden_states (dtype)", hidden_states.dtype) # torch.float16
    if hasattr(self, "store_feature_map"):
        self.feature_map = hidden_states
        # print("feature_map", self.feature_map.shape) # [2, 218, 1536]
    ####################################################################################################
